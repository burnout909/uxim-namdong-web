...
export default async function AnnouncementPage({
  ...
  const { notices, totalPages, total } = await getNotices(currentPage);
            ...
            notices.map((notice) => (
              <div key={notice.id} className="px-6 py-4">
                <div className="grid grid-cols-12 gap-4 items-center text-sm">
                  ...
                  <div className="col-span-2 text-gray-500">{notice.views ?? 0}</div>
                </div>
              </div>
            ))
          )}
      ...
      {totalPages > 1 && (
        <div className="mt-8 flex justify-center">
          <nav className="flex space-x-1">
            {currentPage > 1 && (
              <Link
                href={`/notice/announcement?page=${currentPage - 1}`}
            ...
            {Array.from({ length: Math.min(9, totalPages) }, (_, i) => {
              ...
              return (
                <Link
                  key={pageNum}
                  href={`/notice/announcement?page=${pageNum}`}
                  className={`px-3 py-2 text-sm rounded ${
                    isCurrent ? 'bg-blue-500 text-white' : 'text-gray-700 hover:bg-gray-100'
                  }`}
                >
                  {pageNum}
                </Link>
