...
async function getProduct(page = 1, limit = 10) {
...
export default async function JobInfoPage({
  ...
  const { product, totalPages, total } = await getProduct(currentPage);

  return (
    <div className="px-6 py-12 max-w-screen-lg mx-auto">
      {/* 헤더 */}
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-gray-900">생산품</h1>
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        <div className="divide-y divide-gray-100">
          {product.length === 0 ? (
            <div className="text-center py-12 text-gray-500">등록된 생산품이 없습니다.</div>
          ) : (
            product.map((product) => (
              <div key={product.id} className="px-6 py-4">
                <div className="grid grid-cols-12 gap-4 items-center text-sm">
                  ...
                  <div className="col-span-7">
                    <Link
                      href={`/notice/job-info/${product.id}`}
                  ...
                  <div className="col-span-2 text-gray-500">{product.views ?? 0}</div>
                </div>
              </div>
            ))
          )}
      ...
      {totalPages > 1 && (
        <div className="mt-8 flex justify-center">
          <nav className="flex space-x-1">
            {currentPage > 1 && (
              <Link
                href={`/notice/products?page=${currentPage - 1}`}
            ...
            {Array.from({ length: Math.min(9, totalPages) }, (_, i) => {
              ...
              return (
                <Link
                  key={pageNum}
                  href={`/notice/products?page=${pageNum}`}
