...
async function getPhoto(page = 1, limit = 10) {
...
export default async function FreePage({
    ...
    const { photoList, totalPages, total } = await getPhoto(currentPage);

    return (
        <div className="px-6 py-12 max-w-screen-lg mx-auto">
            {/* 헤더 */}
            <div className="flex items-center justify-between mb-8">
                <h1 className="text-2xl font-bold text-gray-900">사진게시판</h1>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {photoList.map((photo) => {
                            const imgSrc =
                                ...
                                (photo.thumbnail_key as unknown as string) || (photoPlaceholder as unknown as { src: string }).src;
                            return (
                                <Link
                                    key={photo.id}
                                    href={`/resources/photo/${photo.id}`}
