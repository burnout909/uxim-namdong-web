...
export default async function PhotoDetail({
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      {/* 상단 탭/사이드바는 레이아웃에서 처리하는 게 깔끔합니다 */}

      {/* 제목 */}
      <h1 className="text-3xl font-bold text-gray-900">사진게시판</h1>
      ...
      <div className="mt-6 flex items-center justify-end gap-2">
        <Link
          href={prev ? `/resources/photo/${prev.id}` : "#"}
          aria-disabled={!prev}
        ...
        <Link
          href={next ? `/resources/photo/${next.id}` : "#"}
          aria-disabled={!next}
          className={`inline-flex items-center gap-2 rounded border px-4 py-2 text-sm ${
            next
              ? "text-gray-700 hover:bg-gray-100"
