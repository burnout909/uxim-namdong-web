...
export default async function VideoBoardDetail({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const result = await getPostDetail("VIDEO", id);
  if (!result) notFound();
  ...
  return (
    <div className="min-w-[929px] mx-auto pb-12 px-6">
      <h1 className="text-heading-large">영상게시판</h1>
      <div className="pt-6">
      ...
      <PostNavigator prev={prev} next={next} basePath="/resources/videos" />
    </div>
  );
}
